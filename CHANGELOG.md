...
- **Dynamic UI (Infrastructure Triad)**:
  - Re-engineered the "Auto-Hide" logic. The header now intelligently fades out after a delay (`autoHideDelay`).
...
**The "Network Parity & Performance" Update**

- **Network Parity**: Added the `isup` command to contrast local reachability against global API status.
- **Performance Metrics**: Added the `speed` diagnostic to measure connection stability via latency jitter.
- **Preferences**: Added `config` system using `chrome.storage.local` for persistent user preferences.

## [2.3.1] - 2026-04-24

- **Session Continuity**: Implemented command history replay and infrastructure triad restoration after side panel reloads.
- **Dynamic UX**: Added a ResizeObserver to automatically refit the terminal viewport during dynamic header transitions.
- **State Persistence**: Integrated full state persistence via `chrome.storage.session`.

## [2.3.0] - 2026-04-23

- **Dynamic DKIM**: Refactored the DKIM audit module into a dynamic, heuristic-based system relying on MX and SPF records.
- **CNAME Following**: Integrated a recursive CNAME follower with a depth-limited safety mechanism to resolve true provider endpoints.

## [2.2.0] - 2026-04-21

- **Error Heuristics**: Transformed generic fetch errors into a dynamic, actionable diagnostic system (e.g., DNS_RESOLUTION_FAILED, CONNECTION_REFUSED, TIMEOUT).
- **DNS Resiliency**: Implemented a DNS retry mechanism to resolve intermittent "Double-MX" SERVFAIL issues.
...
**The "Manifest V3 & Atomic" Update**

- **Architecture**: Complete rewrite to Manifest V3 for modern Chrome compatibility.
- **Atomic Modules**: Split the codebase into strictly organized, single-purpose files under 200 lines to ensure performance and reliability.
...
- **Core Commands**: Launched with fundamental tools: `whois`, `dig`, `curl`, and `openssl`.
