...
export function cmdDetailedHelp(cmd, suggestCommand) {
    ...
    const h = {
        ...
        green:()=>formatHelp({name:"green", syntax:"[domain]", descLines:["Environmental Check.", "Queries The Green Web Foundation to see if", "the domain is hosted on green energy."], aliases:null, examples:[{cmd: "green google.com"}]}),
        cookies:()=>formatHelp({name:"cookies", syntax:"[domain]", descLines:["Privacy Audit.", "Extracts session and tracking cookies.", "Flags session cookies missing the HttpOnly attribute."], aliases:null, examples:[{cmd: "cookies google.com"}]}),
