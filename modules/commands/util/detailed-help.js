...
export function cmdDetailedHelp(cmd, suggestCommand) {
    ...
    const h = {
        ...
        switch:()=>formatHelp({name:"switch", syntax:"", descLines:["Switch target to the active browser tab.", "Use this when the tab-switch notification", "bar doesn't appear or was dismissed."], aliases:"sw, tab", examples:[{cmd: "switch", desc: "(adopt active tab)"}]}),
        ...
        speedtest:()=>formatHelp({name:"speedtest", syntax:"[size_mb]", descLines:["Local bandwidth test.", "Downloads a payload from Cloudflare's", "speed API to measure your internet download", "speed. Default size is 10MB, max is 100MB."], aliases:"bandwidth, nettest", examples:[{cmd: "speedtest"}, {cmd: "curl -o /dev/null http://speedtest...", desc: "raw bash mapping"}]}),
    };
